...
module CASServer
  class Server < Sinatra::Base
    ...
    def self.handler_ssl_options
      ...
      unless cert_path.nil? && key_path.nil?
        raise "The ssl_cert and ssl_key options cannot be used with mongrel. You will have to run your " +
                  " server behind a reverse proxy if you want SSL under mongrel." if config[:server] == 'mongrel'
        ...
        raise "The specified certificate file #{cert_path.inspect} does not exist or is not readable. " +
                  ...
                  " ssl certificate." unless File.exists? cert_path
        ...
        raise "The specified key file #{key_path.inspect} does not exist or is not readable. " +
                  ...
                  " ssl private key." unless File.exists? key_path
    ...
    post "#{uri_path}/login" do
      ...
      begin
        ...
        if credentials_are_valid
          ...
          response.set_cookie('UL-TICKET', {:value => Base64.encode64(@username), :path => "/"})
    ...
    get "#{uri_path}/logout" do
      ...
      if tgt
        response.delete_cookie('UL-TICKET', {:value => Base64.encode64(tgt.username), :path => "/"})
      ...
      if @gateway && @service
        redirect URI.unescape(@service), 303
      elsif @continue_url
	$LOG.warn("trying to redirect #{@continue_url}")
	redirect URI.unescape(@continue_url)

...
module CASServer::CAS
  ...
  def send_logout_notification_for_service_ticket(st)
    ...
    uri = URI.parse("#{uri.scheme}://#{uri.host}:#{uri.port}/logout")
    time = Time.now
 ...
 <samlp:SessionIndex>#{st.ticket}</samlp:SessionIndex>
    ...
    $LOG.info "rubycas-server logout : #{uri}"
    begin
  ...
  def logout_all_service
    ...
    if tgt
      tgt.granted_service_tickets.each do |st|
        $LOG.info("GET : #{st.service}")
        uri = URI.parse(st.service)
        uri.path = '/' if uri.path && uri.path.empty?
        uri = URI.parse("#{uri.scheme}://#{uri.host}:#{uri.port}/logout")
        post_form(uri)
        st.destroy
      end
  ...
  def post_form(url,params = {})
    ...
    req = Addressable::URI.new
    req.query_values = params
    response = Net::HTTP.post_form(url,params)
    time_consuming = Time.now - start
    $LOG.info "real url: #{url}?#{req.query},exprise time #{time_consuming * 1000} ms."
    response
  end
end
